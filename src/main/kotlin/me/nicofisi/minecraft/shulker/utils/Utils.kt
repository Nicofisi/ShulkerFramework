...
const val ERROR_PREFIX: String = "&p[Error] &s"
...
val String.colored: String
    get() =
        ChatColor.translateAlternateColorCodes('&', replace(this, mapOf(
                "<primary>" to "&p",
                "<secondary>" to "&s",
...
fun CommandSender.sendError(messagePath: String, vararg arguments: String) {
    ...
    message = message.replace(HARDCODED_ASTERISK, "%")
    sendMessage(ERROR_PREFIX.colored + message)
}

fun CommandSender.sendHardcodedError(message: String) = sendMessage(ERROR_PREFIX.colored + message)
