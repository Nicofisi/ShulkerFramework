...
fun runTaskLaterAsync(delay: Duration, task: () -> Unit): CTask {
    return if (isBungee)
        ...
        Bukkit.getScheduler().runTaskLaterAsynchronously(PluginInfo.Spigot.plugin, enhanceTask(task), delay.toMillis() / 50)
                .run { CTask(this) }
}
...
fun runTaskTimerAsync(delayToFirst: Duration, delayBetween: Duration, task: () -> Unit): CTask {
    return if (isBungee)
        ProxyServer.getInstance().scheduler.schedule(
                PluginInfo.Bungee.plugin, enhanceTask(task), delayToFirst.toMillis(), delayBetween.toMillis(), TimeUnit.MILLISECONDS)
                .run { CTask(this) }
    else
